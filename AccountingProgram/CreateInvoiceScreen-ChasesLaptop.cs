...
namespace AccountingProgram
{
    internal partial class CreateInvoiceScreen : Form
    {
        ...
        public CreateInvoiceScreen(Invoices currInvoice)
        {
            InitializeComponent();
            delInvoice = currInvoice;
            FillInvoice();
        }
        ...
        {
            InitializeComponent();
            //if there is no invoice to be deleted, then set id to -1 for test later on when saving
            delInvoice.SetId(-1);
        }
        ...
        private bool ContinueToSave()       //Checks to see if it is okay to continue on to save
        {
            ...
            if(luxBagQuanTextBox.Text == "" && stanBagQuanTextBox.Text == "" && delBagQuanTextBox.Text == "")
            {
                return false;
            }
            return true;
        ...
        private void saveIncoiceButton_Click(object sender, EventArgs e)
        {
            if (ContinueToSave())
            {
                if(delInvoice.GetId() == -1)  //If there is an invoice to be deleted, then delete it and keep the 
                { 
                ...
                else
                {
                    ...
                    InvoiceDatabase.DeleteInvoice(delInvoice);
                }
                ...
                InvoiceDatabase.AddInvoice(currInvoice);
                FileHandler.SaveFile(InvoiceDatabase.ToFile(), FileHandler.GetInvoiceFileName());
                CloseWindow();
            }
            else
            {
